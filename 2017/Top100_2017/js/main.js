    ...
    dataReady = function () {//This is called as soon as data is loaded
	  ...
	  a.clusterNames = {};
        ...
        a.iterNodes(
			  ...
			  a.clusterNames[b.color] = b.attr.attributes["Industry"]; // Razieh, for showing industry names in group selection part 
...
function configSigmaElements(config) {
    ...
    for (b in sigInst.clusters)
        a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> <b>' + sigInst.clusterNames[b] + '</b> (' + sigInst.clusters[b].length + ' members)</a></div>');
    //a.sort();
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        sigInst.draw(2, 2, 2, 2);
        $GP.info_name.html('<div style="width:12px;height:12px;border:1px solid #fff;background:' + a + ';display:inline-block"></div> <b> ' + sigInst.clusterNames[a] + '</b>');    // Changed by Razieh
        $GP.info_data.hide();
